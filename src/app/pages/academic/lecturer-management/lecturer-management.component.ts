...
export class LecturerManagementComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.items = [{ icon: 'pi pi-home', label: 'Trang chủ', route: '/' }, { label: 'Quản lý cán bộ/giảng viên' }];
  }

  onLoadCreateLecturer(content: TemplateRef<any>) {
  ...
  onDeleteLecturer(content: TemplateRef<any>) {
    this.modalService.open(content, {
      centered: true,
    });
  }
